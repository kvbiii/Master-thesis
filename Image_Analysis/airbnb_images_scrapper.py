...
class AirbnbImagesScrapping:
    ...
    def selenium_connection(self) -> webdriver.Chrome:
        ...
        options.add_argument(
            r"--user-data-dir={}".format(os.environ["Chrome_User_Data"])
        )
    ...
    def scrape_images(self, listing_url: str) -> Union[List[str], str]:
        ...
        try:
            ...
            return {"images": images, "room_types": room_types}
        except:
        ...
        try:
            ...
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(
                    (By.XPATH, show_all_images_button_x_path)
                )
            )
        except:
            try:
                ...
                WebDriverWait(self.driver, 3).until(
                    EC.presence_of_element_located((By.XPATH, image_x_path))
                )
                return "Image found"
            except:
                raise Exception(
                    "The show all images button is not loading or the image is not loading"
                )
        try:
            show_all_images_button = self.driver.find_element(
                "xpath", show_all_images_button_x_path
            )
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(show_all_images_button)
            )
    ...
    def _get_images_section(self) -> webdriver.Chrome:
        ...
        try:
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located((By.XPATH, images_section_xpath))
            )
    ...
    def _scroll_and_collect_images(self, images_section: webdriver.Chrome) -> List[str]:
        ...
        while True:
            ...
            if images_scroll[-1] == last_image:
                ...
                counter = 0
        final_images, final_room_types = self._remove_duplicates(
            all_images, all_room_types
        )
        return final_images, final_room_types

    def _get_images_from_section(
        self, section: webdriver.Chrome
    ...
    def _remove_duplicates(
        self, all_images: List[str], all_room_types: List[str]
    ) -> List[str]:
        """
